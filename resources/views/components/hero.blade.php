...
<section class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-navy-700 via-navy-600 to-teal-600 px-6 py-14 text-white lg:px-12 lg:py-20">
    {{-- Subtle decorative pattern --}}
    <div class="p
             o  inter-events-none absolute inset-0 opacity-10">
        <div class="absolute -right-16 -top-16 h-64 w-64 rounded-full bg-white/20 blur-3xl"></div>
        <div class="absolute -bottom-24 -left-12 h-80 w-80 rounded-full bg-teal-300/30 bl
               u        r-3xl"></div>
     </div>

    <div class="relative z-10 grid gap-10 lg:grid-cols-2 lg:items-center">
        <div data-aos="fade-up">
            <span class="mb-3 inline-flex items-center gap-2 rounded-full bg-white/15 px-4 py-1.5 text-xs font-semibold uppercase tracking-wide backdrop-blur">
            ...
            <div class="mt-8 flex flex-wrap items-center gap-3">
                <a href="{{ $ctaUrl }}" target="_blank" rel="noopener" data-hotline-track class="btn btn-lg gap-2 border-0 bg-[#25D366] text-white shadow-lg shadow-green-500/30 transition-all hover:bg-[#1fba59] hover:shadow-xl hover:shadow-green-500/40">
            ...
            <div class="grid grid-cols-2 gap-3" data-aos="fade-left" data-aos-delay="100">
                @foreach($stats as $label => $value)
                    <div class="rounded-2xl bg-white/10 p-4 backdrop-blur-sm transition hover:bg-white/15">
                        ...
                        <p class="mt-2 text-2xl font-bold lg:text-3xl">{{ is_numeric($value) ? number_format($value) : $value }}</p>
                    </div>
                @endforeach
            </div>
        @endif
